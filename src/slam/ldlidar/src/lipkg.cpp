...
// #ifndef M_PI
// #define M_PI 3.14159//265358979323846
// #endif
//   bool reversed = (angle_max > angle_min);
//   if ( reversed ) {
//     output.angle_min =  M_PI - angle_max;
//     output.angle_max =  M_PI - angle_min;
//   } else {
//     output.angle_min =  M_PI - angle_min;
//     output.angle_max =  M_PI - angle_max;
//   }

  output.angle_increment = angle_increment;
  // output.scan_time = 0.0;
  ros::Time start_scan_time = output.header.stamp;
  static ros::Time last_scan_time;
  output.scan_time = (start_scan_time - last_scan_time).toSec();
  last_scan_time = start_scan_time;
  

  // output.time_increment = 0.0;
  output.time_increment = output.scan_time / (double)(beam_size-1);
  ...
  for (auto point : src)
  {
	...
	int index = (int)((angle ) / output.angle_increment);
...
//      	output.ranges[index] = range/1000;
//      }   
//      else
...
//    }

	{
      /*If the current content is Nan, it is assigned directly*/
		...
		{
			output.ranges[beam_size -1 - index] = range/1000;
		}   
		else
		{/*Otherwise, only when the distance is less than the current value, it can be re assigned*/
